...
void Magic::toScene(const QJsonObject sceneJsObj)
{
...
void Magic::toPlayers(const QJsonArray playersJsonArr)
{
    foreach (QJsonValue playerValue, playersJsonArr) {
        ...
        if (isMine(nickname)) {
            updateMinePlayer(playerCfg, QPair<int, int>(playerJsObj.value("life").toInt(),
                                                        playerJsObj.value("score").toInt()));
        }

        const QMap<QString, qreal> position = _camera.setPositionObjects(playerCfg.first());

        if (!_players.contains(nickname)) {
            createPlayer(nickname, position, playerCfg);
        }

        _players[nickname]->setRotation(playerJsObj.value("rotation").toDouble());

        if (isStop(QPointF(playerCfg.first()["x"], playerCfg.first()["y"]), _players[nickname]->pos())) {
            return;
        }
...
void Magic::toBullets(const QJsonArray bulletsJsArr)
{
...
void Magic::updateMinePlayer(const QList<QMap<QString, qreal>> playerCfg, const QPair<int, int> dataHud)
{
    _camera.setSizePlayer(playerCfg.last());
    _camera.setOffsetFactor(playerCfg.first(), _scene->sceneRect());

    toHealth(dataHud.first);
    toScore(dataHud.second);
}

void Magic::createBullet(const QJsonValue bulletValue, const QMap<QString, qreal> position,
...
void Magic::createPlayer(const QString &nickname, const QMap<QString, qreal> position,
                         const QList<QMap<QString, qreal> > playerCfg)
{
...
bool Magic::isOutView(const QMap <QString, qreal> position)
{
    const QRectF gScene = _gameScene["scene"]->rect();

    return gScene.width() + 200 < position["x"]
            || gScene.height() + 200 < position["y"]
            || position["x"] < -200
            || position["y"] < -200;
}
...
{
    toScene(dataJsonObj.value("scene").toObject());
    toPlayers(dataJsonObj.value("players").toArray());
    toBullets(dataJsonObj.value("bullets").toArray());
}
...
{
    if (!_players.contains(nickname)) {
        qWarning() << "Warning! Player" << nickname << "is not exist for delete from game scene!";
...
{
    if (!_bullets.contains(id)) {
        qWarning() << "Warning! Bullet" << id << "is not exist for delete from game scene!";
